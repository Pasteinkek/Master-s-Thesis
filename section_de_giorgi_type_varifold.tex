...
\Cref{de_giorgi_solution_to_mmcf} and in the two-phase case, we can think of 
the 
...
\begin{equation}
	...
	\mu_{ t } \coloneqq
	2 \hm^{ 1 } |_{ l } \otimes \left(  \frac{ 1 }{ 2 } \delta_{ ( 1 , 0 ) } + 
	 \frac{ 1 }{ 2 }\delta_{ ( - 1 , 0 ) }\right)_{ x \in \flattorus } ,
\end{equation}
...
\begin{definition}[De Giorgi type varifold solution for multiphase mean 
	curvature flow]
...
In the definition of $ \mu_{ t } $, the factor $1/2$ is present since every 
...
\Cref{figure_interfaces_collapse}, since the equation becomes trivial for $ i = 
j $. 
...
\Cref{mean_curvature_vector_does_not_have_to_point_in_normal_direction}, but 
...
\begin{example}
	...
	\begin{align*}
		...
		\dd{ x_{ 1 } },
	\end{align*}
	which implies that $ H $ is given on $ [ 0 , \Lambda ) \times \{ 0 \} $ $ 
