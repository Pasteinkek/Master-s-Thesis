...
\begin{proof}
	\begin{description}[wide=0pt]
		...
		Fix some $ M > 0 $. Then we decompose the integral
		\begin{equation*}
			\int \abs{ u_{ \varepsilon } - u }^{ 2 } \dd{ x }
			=
			\int_{ \{ \abs{ u_{ \varepsilon } - u } \geq M \} }
				\abs{ u_{\varepsilon } - u }^{ 2 }
			\dd{ x }
			+
			\int_{ \{ \abs{ u_{ \varepsilon } - u } < M \} }
				\abs{ u_{ \varepsilon } - u }^{ 2 }
			\dd{ x }.
		\end{equation*}
		...
		\begin{equation*}
			...
			\to 0
		\end{equation*}
		as $ \varepsilon \to 0 $ since $ \lm^{ d }\left( \abs{ u_{\varepsilon } - u } \geq M \right) \to 0 $ uniformly in time by step 3 and $ u_{\varepsilon }^{ 2 } + 1 $ is equiintegrable uniformly in time by step 4.
		
		For the second summand, take any $ M > \delta > 0 $. then we further decompose
		\begin{align*}
			\sup_{ 0 \leq t \leq T }
				\int_{ \{ \abs{ u_{ \varepsilon } - u } < M \} }
					\abs{ u_{ \varepsilon } - u }^{ 2 }
			...
			\delta^{ 2 } \Lambda^{ d }
			+
			M^{ 2 } \lm^{ d } \left( \abs{ u_{ \varepsilon } - u } > \delta \right).
		\end{align*}
		Taking the limes superior as $ \varepsilon $ tends to zero of this inequality yields that the right hand side can be made arbitrarily small, which finally yields
		\begin{equation*}
			\sup_{ 0 \leq t \leq T }
				\int
					\abs{ u_{ \varepsilon } - u }^{ 2 }
