...
As often in the Calculus of Variations, convergence of energies boost our 
...
the following Proposition, which ensures that we have square-integrable normal 
velocities.
...
\begin{proposition}
	...
	\begin{equation*}
		...
		\energy^{ 0 },
	\end{equation*}
	where $ \energy^{ 0 } \coloneqq \liminf_{ \varepsilon \to 0 } \energy_{ 
	\varepsilon } ( u_{ \varepsilon }^{ 0 } ) $.
\end{proposition}
...
\begin{proof}
	...
	( ( 0 , T ) \times \flattorus )$. 
	Then via the $ \lp^{ 1 } $-convergence of $ \psi_{ \varepsilon } \to \psi 
	$, we have
	\begin{align*}
...
We finish this section with a proof for the equipartition of the energy, which 
tells us that both the summand involving the potential $ W ( u_{ \varepsilon } 
) $ and the norm of the gradient contribute to the energy in similar parts.

\begin{lemma}
	\label{equipartition_of_energies}
	Under the energy convergence assumption (\ref{energy_convergence}), we have 
	for any continuous function $ \varphi \in \cont^{ \infty } ( \flattorus ) $ 
	that
	\begin{align*}
		\energy ( u ; \varphi )
		=
...
\begin{proof}
	...
	equality, we first assume that $ \varphi \in \cont \left( \flattorus ; [ 0 
	, \infty ) \right)$.
	...
	The third and fourth inequality follow for a given non-negative $ \varphi 
	\in \cont \left( \flattorus ; [ 0 , \infty ) \right) $ by the $ \lp^{ 2 } $ 
	estimate
	\begin{align*}
		& \lim_{ \varepsilon \to 0 }
		\int
...
\subsection{Convergence to a $\bv$-solution to twophase mean curvature flow}

We start by definining a BV-formulation for motion by mean curvature.

\begin{definition}[$\bv$-solution to twophase mean curvature flow]
	\label{motion_by_mcv}
	...
	\begin{equation*}
		...
		\right)
	\end{equation*}
	with $ \esssup_{ 0 \leq t \leq T } \energy ( \chi ) $ \emph{moves by mean 
	curvature} if there is a normal velocity
...
\begin{theorem}
	...
	\right) 
	$ such that $ u_{ \varepsilon }^{ 0 } \to u^{ 0 } = ( 1- \chi^{ 0} ) \alpha 
	...
	\begin{equation*}
		...
		\infty,
	\end{equation*}
	we have that that for some subsequence of solutions to 
	(\ref{allen_cahn_eq}) $ u_{\varepsilon } $, there exists a pointwise 
...
\begin{align*}
	...
	\dd{ x }.
\end{align*}
We therefore test equation (\ref{ac_weak_equation}) against $ \varphi_{ 
...
Modica in \cite{luckhaus_modica_gibbs_thompson_relation}.
...
\varepsilon  } } $ by the chain rule, the former integral can be written as
\begin{equation*}
...
will later see a quantitative version of this in 
\Cref{quantitative_reshetnyak}. Here it yields that since $ \abs{ \nabla \psi_{ 
...
\varepsilon } \coloneqq \varepsilon \nabla u_{ \varepsilon } \approx \nu $ in a 
weak sense. 
...
the support of $ \xi $. Then the error we make can be estimated for all $ 
...
Since we are taking $ \varepsilon \to 0 $, we thus only have to make sure that 
...
(\ref{energy_dissipation_sharp}) and \Cref{existence_of_velocity_twophase}.
