...
\begin{equation}
	...
	\sigma \int \abs{ \nabla \chi }
\end{equation}
as already motivated in \Cref{section_mcf}. We will go into more depth why 
...
\begin{align*}
	...
	\limsup_{ \varepsilon \to 0 }
		...
		\dd{ t }.
\end{align*}
The first summand goes to zero via assumption, and the second vanishes as well 
...
\begin{theorem}
	...
	$ \chi^{ 0 } $ in the sense of \Cref{motion_by_mcv}.
\end{theorem} 
...
\begin{align*}
	...
	\dd{ t }.
\end{align*}
But how do we find these test functions? For this, we first note that the 
curvature term
...
In \Cref{existence_of_ac_solution}, we have shown $ u_{ \varepsilon } \in 
...
for now.
Since our normal is frozen, we notice that via the equipartition of energies, 
we may replace $ \varepsilon \abs{ \nabla u_{ \varepsilon } } $ with $ \sqrt{ 2 
W 
...
(\Cref{equipartition_of_energies}).
But now we recognize by the chain rule that
\begin{equation*}
	\int_{ 0 }^{ T }
	\int
	\partial_{ t } u_{ \varepsilon }
...
(\ref{rewriting_limit_of_frozen_velocity_term}). As in inequality 
(\ref{acceptable_error_term_velocity_twophase}) this can be estimated via 
Young's inequality by the error
\begin{equation*}
	\norm{ \xi }_{ \lp^{ \infty } }
	\left(
	\alpha
...
\end{align*}
But by the equipartition of energies (\Cref{equipartition_of_energies}), it 
holds that
\begin{equation*}
	2 \int_{ 0 }^{ T }
	\int
...
Therefore let us now argue why the error can be made arbitrarily 
small. 
